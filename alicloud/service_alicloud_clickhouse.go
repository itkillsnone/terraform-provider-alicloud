...
func (s *ClickhouseService) DescribeDBClusterColdStorage(id string, regionId string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewClickhouseClient()
	if err != nil {
		return nil, WrapError(err)
...
func (s *ClickhouseService) ClickHouseColdStorageStatusRefreshFunc(id string, regionId string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := s.DescribeDBClusterColdStorage(id,regionId)
		if err != nil {
			if NotFoundError(err) {
				// Set this to nil as if we didn't find anything.
				return nil, "", nil
